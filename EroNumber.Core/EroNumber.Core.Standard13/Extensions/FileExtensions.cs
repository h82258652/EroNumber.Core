...
namespace EroNumber.Extensions
{
    public static class FileExtensions
    {
        ...
        public static async Task WriteAllBytesAsync(string path, byte[] bytes, CancellationToken cancellation)
        {
            if (path == null)
            {
                throw new ArgumentNullException(nameof(path));
            ...
            {
                await fs.WriteAsync(bytes, 0, bytes.Length, cancellation);
            }
        }
